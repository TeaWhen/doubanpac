$.ajax({
  ...
  error: function() {
    console.log("out ZJU");
    checkPac("http://webpac.zju.edu.cn/");
  },
  complete: function() {
...
function checkPac(base_url) {
  ...
  var find_xml = new JKL.ParseXML( base_url+"X?op=find&base=zju01&code=wrd&request=" + book_isbn );
  var find_data = find_xml.parse();

  if (find_data.find.no_records > 0) {
    var book_xml = new JKL.ParseXML( base_url+"X?op=present&set_no="+find_data.find.set_number+"&set-entry=1" );
    var book_data = book_xml.parse();

    var pac_xml = new JKL.ParseXML( base_url+"X?op=item-data&base=zju01&doc_number="+book_data.present.record.doc_number )
  ...
  $('.doubanpac-send').click(function() {
    ...
    chrome.extension.sendMessage({method: "getServerInfo"}, function(response) {
      if (response.username){
        ...
        alert("Please set Remarque settings.");
      };
    });
    return false;
